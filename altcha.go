...
// altchaSweepEvery bounds how often a refusal may sweep the store. The sweep itself
// is load bearing - it readmits clients whose entries lapsed since the last poll
// tick - but pruneLocked walks every entry while holding s.mu, which at capacity is
// 200k iterations, and it used to run on EVERY refused request. Measured on a full
// store that collapsed the endpoint to 68 req/s and pushed a warm client already
// holding a challenge from 230us to 27ms at two refused clients and 458ms at
// thirty-two: the attacker's cost fell (no derivation) while the daemon's rose (a
// full scan), which inverts the point of bounding mints. One sweep a second keeps
// the readmission and drops the amplification.
const altchaSweepEvery = time.Second

// errAtCapacity is returned when altchaMaxLive is reached. It is a sentinel rather
// than an inline error so the caller can separate "the store is full" - the one
// failure on this path an unauthenticated caller can provoke on purpose, and so the
// one worth counting - from a config or entropy fault, which is a system problem and
// would make that counter lie about an attack.
var errAtCapacity = errors.New("too many challenges outstanding")
...
type altchaStore struct {
	...
	full bool
	// lastSweep throttles the pre-refusal sweep; see altchaSweepEvery. A plain field
	// rather than dueAgain's atomic.Int64: every reader and writer already holds
	// s.mu, so there is no concurrency for a CAS to resolve, and second granularity
	// is too coarse to bound how often an O(len(live)) scan may run.
	lastSweep time.Time
}
...
func (s *altchaStore) challengeFor(ip, algorithm string, cost int, maxCounter int64, now time.Time) (altchaChallenge, error) {
	...
	for {
		...
		if !replacing && len(s.live) >= altchaMaxLive {
			// prune otherwise only runs on the poll tick, so sweep before refusing -
			// but at most once per altchaSweepEvery, because this walks every entry
			// under s.mu and a refused request must not be able to buy that scan.
			if now.Sub(s.lastSweep) >= altchaSweepEvery {
				s.lastSweep = now
				s.pruneLocked(now)
				_, replacing = s.live[ip]
			}
		...
		if !replacing && len(s.live) >= altchaMaxLive {
			...
			return altchaChallenge{}, errAtCapacity
		}
		s.full = false
