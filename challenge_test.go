...
func TestSafeReturn(t *testing.T) {
	cases := map[string]string{
		...
		// Not valid UTF-8, so it cannot survive the redirect: Chrome replaces each
		// bad byte with U+FFFD and requests the mangled path, which 404s. Observed
		// live with /a%e0%80%af (an overlong "/") landing on
		// /a%ef%bf%bd%ef%bf%bd%ef%bf%bd. The root at least works.
		"/a\xe0\x80\xaf": "/", // overlong encoding of "/"
		"/a\xc0\xaf":     "/", // the shorter overlong form
		"/caf\xe9.html":  "/", // a real Latin-1 filename, not an attack
		"/a\xff\xfe":     "/", // a stray BOM in the wrong encoding
		// Valid multi-byte UTF-8 must still round-trip untouched.
		"/caf\u00e9":          "/caf\u00e9",
		"/\u65e5\u672c\u8a9e": "/\u65e5\u672c\u8a9e",
	}
...
func TestChallengeRendersThePage(t *testing.T) {
	...
	for _, want := range []string{
		`<altcha-widget`,
		// Pinned because nothing else would notice its loss: the widget renders and
		// solves identically with the footer showing, so a dropped attribute is only
		// visible to somebody looking at the page. Asserted in its ESCAPED form,
		// because that is what reaches the browser - and the first attempt at this
		// used hidefooter="true", which the widget silently ignores because it is not
		// one of the nine attributes create_custom_element registers.
...
// The one failure in this store that fails OPEN. Apache honours the file, not the
// map, so a pass the daemon believes it expired is still being enforced until the
// write lands. Dropping it from memory anyway left the next prune with nothing to
// find, and prune's own removed == 0 guard then returned early on a file that still
// named it - so the address stayed exempt for the life of the process, with the
// gauge reading zero and one log line at the moment it broke.
//
// Nothing new happens between the two prunes here: no solve, no deletion, no reset.
// The next ordinary poll has to be the whole recovery path.
func TestPassStorePruneRetriesAfterAFailedWrite(t *testing.T) {
	if os.Geteuid() == 0 {
		t.Skip("running as root: an unwritable directory does not stop the write")
	}
	dir := t.TempDir()
	path := filepath.Join(dir, "captcha_passed.txt")
	store := newPassStore(path, time.Minute)
	now := time.Now()
	if err := store.add("203.0.113.9", now); err != nil {
		t.Fatal(err)
	...
	// writeLocked renders through a temp file and a rename, both in the parent
	// directory, so taking write permission off the directory is what fails it.
	if err := os.Chmod(dir, 0o500); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.Chmod(dir, 0o755) }) // or TempDir cannot clean up
	removed, err := store.prune(now.Add(2 * time.Minute))
	if err == nil {
		t.Fatal("prune reported success with the map directory unwritable")
	}
	if removed != 0 {
		t.Errorf("prune reported %d removed on a write that failed; that credits passes_expired for a pass the file still names", removed)
	}
	if store.held() != 1 {
		t.Errorf("held = %d, want 1: the pass is still in the file, so it is still being honoured", store.held())
	}

	if err := os.Chmod(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	removed, err = store.prune(now.Add(3 * time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	if removed != 1 {
		t.Errorf("the retry removed %d, want 1", removed)
	}
	got, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	...
	if len(got) != 0 {
		t.Fatalf("the lapsed pass is still exempt after the fault cleared: %q", got)
	}
}
...
// Passes are enforced by Apache whether or not the daemon is running: the refuse
// rule consults ${solved:...} straight off disk. So a pass map left populated at
// shutdown keeps exempting addresses that no process holds an expiry for, for the
// whole outage - the one window "when it is down we refuse" exists to cover. The
// readiness file fails closed there because systemd deletes /run/... on stop;
// nothing cleans /var/lib, so the pass map has to be cleared deliberately.
//
// The file must still EXIST afterwards. Apache opens a txt: RewriteMap at
// config-parse time, so removing it would trade a stale-pass window for "Apache
// will not start" - see startChallenge.
//
// This drives the same sequence run does, in run's order, but synchronously: adding
// a pass from a second goroutine would race run's own assignment of b.passes and
// prove nothing except that the test is racy. What it therefore does NOT cover is
// the one-line `defer b.stopChallenge()` in run that ties it to shutdown.
func TestStopChallengeEmptiesThePassMapButKeepsTheFile(t *testing.T) {
	dir := t.TempDir()
	b := testBouncer(t, func(c *config) {
		c.bouncingOnType = bouncingAll
		c.captchaListen = "" // no listener: the pass map is created either way
		c.captchaPassFile = filepath.Join(dir, "captcha_passed.txt")
		c.captchaPassTTL = time.Hour
	})

	b.startChallenge(context.Background())
	if b.passes == nil {
		t.Fatal("the pass map must be created even with the listener off, or Apache cannot parse its config")
	}
	if err := b.passes.add("203.0.113.9", time.Now()); err != nil {
		t.Fatalf("recording a pass: %v", err)
	}
	if got, _ := os.ReadFile(b.cfg.captchaPassFile); !strings.Contains(string(got), "203.0.113.9") {
		t.Fatalf("the pass should be published before shutdown, got %q", got)
	}

	b.stopChallenge()

	got, err := os.ReadFile(b.cfg.captchaPassFile)
	if err != nil {
		t.Fatalf("the pass map must survive shutdown as an empty file, not be removed: %v", err)
	}
	if len(got) != 0 {
		t.Fatalf("pass map still exempts addresses after shutdown: %q", got)
	}
}
